...
while True: 
    ...
    target = (n//k)*k # 나누는게 1 빼는 것보다 더 효율적이니까 나누어 떨어지는 가장 큰 수를 구하고
    result += (n-target) # n에서 나누어 떨어지는 가장 큰 수를 빼면 1을 빼는 연산을 몇 번 해야되는지 나오니 1을 빼는 연산의 횟수를 result에 더해주고
    n=target # n을 나누어 떨어지는 가장 큰 수인 target으로 바꿔주고
...
result += (n-1) # n이 1보다 큰 경우가 있을 수 있으므로 n이 1이 될 때까지 n에서 1을 빼주는 연산의 횟수를 result에 더해줌
print(result)
end_time = time.time() # 측정 종료
print("time:", end_time - start_time) # 수행 시간 출력
