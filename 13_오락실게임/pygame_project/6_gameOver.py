...
# 화면 크기 설정
screen_width = 640 # 가로 폭
screen_height = 480 # 세로 높이
screen = pygame.display.set_mode((screen_width, screen_height))

# 화면 타이틀 설정
pygame.display.set_caption("HS Pang") # 게임 이름
    ...
    for event in pygame.event.get(): 
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                ...
                character_to_x += character_speed
            elif event.key == pygame.K_SPACE: # 무기 발사(스페이스 키)
                weapon_x_pos = character_x_pos + (character_width / 2) - (weapon_width / 2)
                weapon_y_pos = character_y_pos
                weapons.append([weapon_x_pos, weapon_y_pos]) # 무기의 x, y 좌표 리스트를 weapons 리스트에 삽입
        ...
        for weapon_idx, weapon_val in enumerate(weapons): 
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3: # 가장 작은 크기의 공이 아니라면 다음 단계의 공으로 나눠주기
                    ...
                    balls.append({ # 오른쪽으로 튕겨 나가는 작은 공
                        "pos_x" : ball_pos_x + (ball_width / 2) - (small_ball_width / 2), # 공의 x 좌표
                        "pos_y" : ball_pos_y + (ball_height / 2) - (small_ball_height / 2), # 공의 y 좌표
                        "img_idx" : ball_img_idx + 1, # 공의 이미지 인덱스
                        "to_x" : 3, # x축 이동방향, -3이면 왼쪽으로, 3이면 오른쪽으로 
            ...
            continue # 안쪽 for 문의 조건의 맞지 않으면 continue, 바깥 for 문을 계속 수행
